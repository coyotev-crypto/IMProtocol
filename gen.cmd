...
for %%i in (%PROTO_NAMES%) do (
    protoc --go_out=./%%i --go_opt=module=github.com/coyotev-crypto/IMProtocol/%%i %%i/%%i.proto
    if ERRORLEVEL 1 (
...
for %%i in (%PROTO_NAMES%) do (
    protoc --go-grpc_out=./%%i --go-grpc_opt=module=github.com/coyotev-crypto/IMProtocol/%%i %%i/%%i.proto
    if ERRORLEVEL 1 (
        echo error processing %%i.proto
        exit /b %ERRORLEVEL%
    )
